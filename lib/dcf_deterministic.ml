...
let classify_growth_relationship ~empirical ~implied : string =
  let diff = empirical -. implied in
  let pct_diff = (diff /. (abs_float implied +. 1e-8)) *. 100.0 in
  if abs_float pct_diff < 1.0 then
    "In line with market expectations"
  else if pct_diff > 0.0 then
    "Market price implies slower growth; model inputs are more aggressive by comparison"
  else
    "Market price implies faster growth; model inputs are more conservative by comparison"
;;
...
let format_large_number value =
  ...
  let formatted =
    if abs_val >= 1.0e9 then Printf.sprintf "%.2fB" (abs_val /. 1.0e9)
    else if abs_val >= 1.0e6 then Printf.sprintf "%.2fM" (abs_val /. 1.0e6)
    else Printf.sprintf "%.2f" abs_val
  in
  if value < 0.0 then "-" ^ formatted else formatted
;;
...
let format_currency ~currency value =
  ...
  let abs_val = abs_float value in
  let formatted =
    if abs_val >= 1.0e9 then Printf.sprintf "%.2fB%s" (abs_val /. 1.0e9) suffix
    else if abs_val >= 1.0e6 then Printf.sprintf "%.2fM%s" (abs_val /. 1.0e6) suffix
    else Printf.sprintf "%.2f%s" abs_val suffix
  in
  if value < 0.0 then "-" ^ formatted else formatted
;;
...
let display_valuation_summary
  ...
  Printf.printf "\n\n===============================================================\n%!";
  Printf.printf "==== Valuation for %s (%s) ====\n" ticker long_name;

  if capped_fcfegr then
    Printf.printf "[WARN] FCFEGR (%.4f) capped to upper bound (%.4f)\n%!" fcfegr fcfegr_clamp_upper;
  if floored_fcfegr then
    Printf.printf "[WARN] FCFEGR (%.4f) floored to lower bound (%.4f)\n%!" fcfegr fcfegr_clamp_lower;

  if capped_fcffgr then
    Printf.printf "[WARN] FCFFGR (%.4f) capped to upper bound (%.4f)\n%!" fcffgr fcffgr_clamp_upper;
  if floored_fcffgr then
    Printf.printf "[WARN] FCFFGR (%.4f) floored to lower bound (%.4f)\n%!" fcffgr fcffgr_clamp_lower;
  ...
  Printf.printf "\n==== Projected vs Implied Growth Rates (Years 1–%d) ====\n" h;
  Printf.printf "→ FCFE:\n";
  Printf.printf "   - Empirically derived (used for projection): %.2f%% per year\n" (avg_fcfegr *. 100.0);
  Printf.printf "   - Implied by market price: %.2f%% per year\n" (implied_fcfegr *. 100.0);
  Printf.printf "   - Assessment: %s\n"
    (classify_growth_relationship ~empirical:avg_fcfegr ~implied:implied_fcfegr);

  Printf.printf "→ FCFF:\n";
  Printf.printf "   - Empirically derived (used for projection): %.2f%% per year\n" (avg_fcffgr *. 100.0);
  Printf.printf "   - Implied by market price: %.2f%% per year\n" (implied_fcffgr *. 100.0);
  Printf.printf "   - Assessment: %s\n"
    (classify_growth_relationship ~empirical:avg_fcffgr ~implied:implied_fcffgr);
