...
def load_histories(data_dir: Path) -> dict[str, pd.DataFrame]:
    ...
    for source, store in (("thetadata", td_frames), ("yfinance", yf_frames)):
        for f in sorted(data_dir.glob(f"*_earnings_vol_{source}.csv")):
            ticker = f.stem.replace(f"_earnings_vol_{source}", "")
            try:
                df = pd.read_csv(f)
                if not df.empty:
    ...
    for ticker in set(td_frames) | set(yf_frames):
        chosen = prefer_thetadata(
            td_frames.get(ticker, []), yf_frames.get(ticker, []), ["date"], 1
        )
