...
Displays and analyzes forward-testing results for pre-earnings straddles.
"""
...
def view_history(filter_recommendation: str = None):
    ...
    if completed > 0:
        ...
        print(f"\nPredicted vs Actual:")
        predicted_avg = completed_df['predicted_return'].mean() * 100
        actual_avg = completed_df['actual_return_pct'].mean()
        print(f"  Predicted avg return: {predicted_avg:.1f}%")
        print(f"  Actual avg return: {actual_avg:.1f}%")
        print(f"  Difference: {actual_avg - predicted_avg:+.1f}%")

        # Top performers
        if len(completed_df) >= 5:
            print(f"\n=== Top 5 Winners ===")
            top_winners = completed_df.nlargest(5, 'actual_return_pct')[
                ['ticker', 'earnings_date', 'actual_return_pct', 'actual_pnl', 'recommendation']
            ]
    ...
    if pending > 0:
        ...
        pending_df = df[df['status'] == 'pending'][
            ['ticker', 'earnings_date', 'days_to_earnings', 'recommendation', 'predicted_return']
