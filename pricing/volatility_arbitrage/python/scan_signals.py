...
def load_histories(data_dir: Path, min_days: int) -> dict[str, pd.DataFrame]:
    ...
    for pattern in ("*_volarb_history_yfinance.csv", "*_volarb_history_thetadata.csv"):
        for f in sorted(data_dir.glob(pattern)):
            ticker = f.stem.replace("_volarb_history_yfinance", "").replace("_volarb_history_thetadata", "")
            try:
                df = pd.read_csv(f)
                if not df.empty:
