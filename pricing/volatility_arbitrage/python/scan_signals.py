...
def load_histories(data_dir: Path, min_days: int) -> dict[str, pd.DataFrame]:
    """Load vol arb history CSVs from both yfinance and thetadata sources."""
    histories = {}
    ...
    for source, store in (("thetadata", td_frames), ("yfinance", yf_frames)):
        for f in sorted(data_dir.glob(f"*_volarb_history_{source}.csv")):
            ticker = f.stem.replace(f"_volarb_history_{source}", "")
            try:
                df = pd.read_csv(f)
                if not df.empty:
    ...
    return filter_stale(histories, "date", "volatility_arbitrage")
