...
def load_histories(data_dir: Path, min_days: int) -> dict[str, pd.DataFrame]:
    """Load IV history per ticker, preferring ThetaData over yfinance.

    Uses a single source per ticker — never concatenates the two: ThetaData alone
    when it has >= min_days rows, else yfinance alone. See
    lib.python.iv_history.prefer_thetadata for why mixing the sources (yfinance
    runs ~4 vol pts hot) corrupts the z-score at the source boundary.
    """
    histories = {}
    ...
    for source, store in (("thetadata", td_frames), ("yfinance", yf_frames)):
        for f in sorted(data_dir.glob(f"*_iv_history_{source}.csv")):
            ticker = f.stem.replace(f"_iv_history_{source}", "")
            try:
                df = pd.read_csv(f)
                if not df.empty:
    ...
    for ticker in set(td_frames) | set(yf_frames):
        chosen = prefer_thetadata(
            td_frames.get(ticker, []), yf_frames.get(ticker, []), ["date"], min_days
        )
        if chosen is not None:
            histories[ticker] = chosen

    return filter_stale(histories, "date", "variance_swaps")
