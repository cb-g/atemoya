...
let compute_d1_d2 ~spot ~strike ~expiry ~rate ~dividend ~volatility =
  if expiry <= 0.0 then
    failwith "Black_scholes.compute_d1_d2: expiry must be positive"
  else if volatility <= 0.0 then
    failwith "Black_scholes.compute_d1_d2: volatility must be positive"
  else if spot <= 0.0 then
    failwith "Black_scholes.compute_d1_d2: spot price must be positive"
  else if strike <= 0.0 then
    failwith "Black_scholes.compute_d1_d2: strike must be positive"
  else
...
*)
let price_european_option ~option_type ~spot ~strike ~expiry ~rate ~dividend ~volatility =
  if expiry <= 0.0 then
...
let calculate_greeks option_type ~spot ~strike ~expiry ~rate ~dividend ~volatility =
  if expiry <= 0.0 then
