...
def load_histories(data_dir: Path, min_days: int) -> dict[str, pd.DataFrame]:
    ...
    for source, store in (("thetadata", td_frames), ("yfinance", yf_frames)):
        for f in sorted(data_dir.glob(f"*_ff_history_{source}.csv")):
            ticker = f.stem.replace(f"_ff_history_{source}", "")
            try:
                df = pd.read_csv(f)
                if df.empty:
                    continue
    ...
    for ticker in set(td_frames) | set(yf_frames):
        chosen = prefer_thetadata(
            td_frames.get(ticker, []), yf_frames.get(ticker, []), ["date", "dte_pair"], min_days
        )
        if chosen is not None:
            histories[ticker] = chosen

    return filter_stale(histories, "date", "forward_factor")
