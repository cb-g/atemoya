(** The human-readable summary of a batch of valuations: counts by status,
    model and failure reason, one line per ticker, and, when the universe is
    given, whether each ticker met its expected outcome. Pure and deterministic
    for a given input order. *)

val reason_key : string -> string
(** A failure reason with its parenthesised specifics dropped, for grouping. *)
