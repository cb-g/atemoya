...
flowchart TD
    ...
    SIG --> DECL{"entity_class declared? (a universe entry, exactly ticker, entity_class, why and scope_limits, loaded strictly, from reference/universe.json or any file of the same format given as --universe, such as a private one under data/ (21); or --entity-class)"}
    ...
    ADM -- "none (RegulatedUtility, MerchantPower, Miner, Royalty, PreProfit, Wrapper, ConstructionStage, UnderBid, Ballast)" --> F_INADM["dcf not admissible for (class); lens: (lens)"]:::failed
    ADM -- "dcf (OperatingCompany; HighGrowthSoftware since 21: the settled reversion is the anchor at conservative persistence and the implied readouts are the reading, stock compensation inside EBIT as filed; PreProfit stays refused, negative free cash flow gives the solvers nothing to invert)" --> COUNTRY
