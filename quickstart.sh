...
show_skew_trading_menu() {
    while true; do
        ...
        echo -e "${DIM}Tip: Run Macro Dashboard (Alternative menu) first for regime-tagged scan signals.${NC}"
        echo ""
        echo -e "${YELLOW}Enter your choice (Enter=Full Workflow):${NC} "
        read -r choice
...
show_pre_earnings_straddle_menu() {
    while true; do
        ...
        echo ""
        echo -e "${DIM}Tip: Run Macro Dashboard for regime tags, NLP Sentiment for directional bias.${NC}"
        echo ""
        echo -e "${GREEN}0)${NC} Back to Pricing Menu"
        echo ""
        echo -e "${YELLOW}Enter your choice (Enter=Full Workflow):${NC} "
