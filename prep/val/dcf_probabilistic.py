...
DATA_DIR = Path("data")
DATA_VAL_DIR = Path("data/val/dcf")
TICKERS_PATH = DATA_DIR / "tickers.yml"
OUTPUT_PATH = DATA_VAL_DIR / "input_for_probabilistic.json"
PARAMS_FILE = DATA_VAL_DIR / "params.yml"
RFR_FILE = DATA_VAL_DIR / "risk_free_rates.yml"
ERP_FILE = DATA_VAL_DIR / "erp_usd.yml"
BETA_FILE = DATA_VAL_DIR / "betas.yml"
INFLATION_FILE = DATA_VAL_DIR / "inflation.yml"
TAX_FILE = DATA_VAL_DIR / "tax_rates.yml"
...
growth_clamp = params.get("growth_rate_clamp", {})
clamp_upper = float(growth_clamp.get("upper", 0.5))
clamp_lower = float(growth_clamp.get("lower", -0.5))
...
def estimate_current_assets_from_balance(balance_sheet: dict) -> float:
    """
    Estimate total current assets by summing available components from balance_sheet.
    Assumes balance_sheet is a yfinance-style DataFrame (field -> column of years).
    """
    components = [
    ...
    for key in components:
        if key in balance_sheet.index:
            vals = balance_sheet.loc[key].dropna()
            if not vals.empty:
                total += float(vals.iloc[0])
                found = True
    if not found:
        raise KeyError("Could not estimate current assets from any components.")
    return total

def estimate_current_liabilities_from_balance(balance_sheet: dict) -> float:
    """
    Estimate total current liabilities by summing available components from balance_sheet.
    Assumes balance_sheet is a yfinance-style DataFrame (field -> column of years).
    """
    components = [
        "Accounts Payable",
    ...
    for key in components:
        if key in balance_sheet.index:
            ...
            if not vals.empty:
                ...
                found = True
    if not found:
        raise KeyError("Could not estimate current liabilities from any components.")
    return total
...
def fetch_valuation_inputs(ticker: str) -> dict:
    ...
    d = extract_from_df_with_fallback(
        ...
        "Reconciled Depreciation", "Depreciation And Amortization In Income Statement", "Depreciation Income Statement"],
        "Depreciation"
    )
    ...
    try:
        ...
        print(f"[WARN] Using component-based fallback to estimate current assets")
        ca = estimate_current_assets_from_balance(balance)
        prev_ca = ca  # fallback: assume no change
    ...
    try:
        ...
        print(f"[WARN] Using component-based fallback to estimate current liabilities")
        cl = estimate_current_liabilities_from_balance(balance)
        prev_cl = cl  # fallback: assume no change
    ...
    # -----------------------------
    # Time series extraction block
    # -----------------------------
    def extract_series(df, keys: list[str], name: str, flip_sign=False) -> list[float]:
        for key in keys:
            if key in df.index:
                vals = df.loc[key].dropna().astype(float)
                if not vals.empty:
                    result = vals.values.tolist()
                    if flip_sign:
                        result = [-v if v > 0 else v for v in result]
                    return result
        return []

    time_series = {
        "ebit": extract_series(income_stmt, ["EBIT", "Operating Income", "Operating Income Before Depreciation", "Income Before Interest and Taxes"], "ebit"),
        "ni": extract_series(income_stmt, ["Net Income", "Net Income Common Stockholders", "Net Income Applicable to Common Shares", "Consolidated Income"], "ni"),
        "capx": extract_series(cashflow, ["Capital Expenditures", "Capital Expenditure", "Purchase Of Property Plant And Equipment",
                                          "Investments In Property Plant And Equipment", "Purchase Of Fixed Assets",
                                          "Additions To Property Plant And Equipment"], "capx", flip_sign=True),
        "d": extract_series(cashflow, ["Depreciation", "Depreciation and Amortization", "Depreciation Amortization Depletion",
                                       "Reconciled Depreciation", "Depreciation And Amortization In Income Statement", "Depreciation Income Statement"], "d"),
        "ca": extract_series(balance, ["Total Current Assets", "Current Assets", "Total Assets Current"], "ca"),
        "cl": extract_series(balance, ["Total Current Liabilities", "Current Liabilities", "Total Liabilities Current"], "cl"),
        "bve": extract_series(balance, ["Common Stock Equity", "Total Equity Gross Minority Interest", "Total Equity"], "bve"),
        "dp": [dp_total] * 4,
        "ic": [bve + mvb] * 4,
    }
